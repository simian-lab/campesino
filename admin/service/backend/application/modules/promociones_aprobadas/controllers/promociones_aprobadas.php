...
class Promociones_aprobadas extends Main {
	...
	function index()
	{
        ...
        $arrCategorias = $this->promociones_aprobadas_model->get_categorias();
        $arrSubCategorias = $this->promociones_aprobadas_model->get_subcategorias();
        $arrMarcas = $this->promociones_aprobadas_model->get_marcas();
        $arrTipoPromocion=array('1'=>'Básica','2'=>'Premium Home','3'=>'Premium Categorías');
        $arrUsersAliados = $this->promociones_aprobadas_model->get_user_aliados();
		...
		if( isset($segmento) && !empty($segmento) ){
				$result = $this->promociones_aprobadas_model->control_promocion($segmento,$this->session->userdata('sadmin_user_id'));
		...
		/*CONTROL*/
  		...
  		$result = $this->promociones_aprobadas_model->get_grupo($this->session->userdata('sadmin_user_id'));
       ...
       $crud->order_by('PRO_FECHA','DESC');
		...
		$crud->fields('PRO_NOMBRE','PRO_SRC_ID','PRO_LOGO_PREMIUM', 'PRO_LOGO_GENERAL','PRO_DESCRIPCION','MAR_ID','CAT_ID','SUB_ID','PRO_PRECIO_INICIAL','PRO_PRECIO_FINAL','PRO_TIPO_MONEDA','PRO_DESCUENTO','VISIBILITY','PRO_USER_CREADOR', 'ID_USER_CREADOR','PRO_URL', 'VISTA_PREVIA','eventos');
        $crud->required_fields('PRO_NOMBRE','PRO_SRC_ID','PRO_DESCRIPCION','PRO_URL', 'CAT_ID','PRO_USER_CREADOR','eventos');
        $crud->columns('PRO_NOMBRE','PRO_AUTOR','CAT_ID','SUB_ID','AUTORIZADO', 'PRO_SRC_ID','VISIBILITY','eventos');
		...
		$crud->field_type('ID_USER_CREADOR', 'invisible');
		...
		$this->data['titulo']='Promociones aprobadas';
		$this->data['encabezado']='Gestión de promociones aprobadas';

		$breadcrums[]='<a class="current" href="'.site_url('main/promociones').'">Promociones</a>';
		$this->salida('promociones_aprobadas/promociones_aprobadas',$this->data, $breadcrums);
	}
...
function get_user_creador($value){
	$result = $this->promociones_aprobadas_model->get_user($value);
	return $result['username'];
}

function get_id_user_creador($value, $id_promo){
	$result = $this->promociones_aprobadas_model->get_user_x_promocion($id_promo);
	return $result['PRO_USER_CREADOR'];
}

function get_marca($value){
	$result = $this->promociones_aprobadas_model->get_marca($value);
	return $result['MAR_NOMBRE'];
}

function get_cat($value){
	$result = $this->promociones_aprobadas_model->getCategoriaId($value);
	return $result['CAT_NOMBRE'];
}

function get_subcat($value){
	$result = $this->promociones_aprobadas_model->getSubCategoriaId($value);
...
function columna_paquete($value, $row){
  //print_R($row);
  $paquete = $this->promociones_aprobadas_model->get_paquete($row->PRO_USER_CREADOR);
  //print_R($paquete);
  return $paquete[$row->PRO_USER_CREADOR];
}

function tipo_promocion($post_array){//print_r($post_array);
  if($post_array == 1){
    return 'General';
  }
  elseif($post_array == 3){
...
$datos= $this->promociones_aprobadas_model->decrementar($pUSER_ID,$pTIPO);
