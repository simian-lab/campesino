...
class formulario_sorteo extends Main {
	...
	function index()
	{
        ...
        $crud->unset_add();
        $crud->unset_edit();
		//$crud->fields('PAU_EVENTO','PAU_ID','PAU_NOMBRE','PAU_IMAGEN','PAU_MOVIL_IMAGEN','PAU_DESCRIPCION','VISIBILITY','Tags','PAU_FECHA','PAU_USER_CREADOR','PAU_USER_ULTIMO');
        //$crud->required_fields('PAU_NOMBRE','PAU_IMAGEN','VISIBILITY','PAU_FECHA','PAU_DESCRIPCION');
        $crud->columns('US_NOMBRE','US_EMAIL','US_DIRECCION','US_CELULAR','US_TIENDA','US_FECHA');
...
function field_callback_image($value,$primary_key){
	...
	return '<a href="'.$primary_key.'/descargarFactura/'.$value.'" id="descargaFactura" />Descargar factura</a>';
