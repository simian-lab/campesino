...
class Promociones_premium extends Main {
	...
	function index()
	{
        ...
        $arrCategorias = $this->promociones_premium_model->get_categorias();
        $arrSubCategorias = $this->promociones_premium_model->get_subcategorias();
        $arrMarcas = $this->promociones_premium_model->get_marcas();
        $arrEventos = $this->promociones_premium_model->get_eventos();


		$crud = new grocery_CRUD();
		$crud->set_theme('flexigrid');
		$crud->where('PRO_SRC_ID','3');
		...
		if( isset($segmento) && !empty($segmento) ){
				$result = $this->promociones_premium_model->control_promocion($segmento,$this->session->userdata('sadmin_user_id'));
	   ...
	   $result = $this->promociones_premium_model->get_grupo($this->session->userdata('sadmin_user_id'));
       ...
       $crud->order_by('PRO_FECHA','DESC');
		...
		$crud->fields('PRO_NOMBRE','PRO_LOGO_PREMIUM','PRO_LOGO_GENERAL','PRO_DESCRIPCION','MAR_ID','PRO_SRC_ID','CAT_ID','SUB_ID','PRO_PRECIO_INICIAL','PRO_PRECIO_FINAL','PRO_TIPO_MONEDA','PRO_DESCUENTO','VISIBILITY','PRO_USER_CREADOR','PRO_USER_ULTIMO','PRO_URL','PRO_AUTOR','PRO_FECHA','AUTORIZADO','PRO_LOGO_VISA','eventos', 'VISTA_PREVIA','PRO_HASH','PRO_ACTIVA');
        $crud->required_fields('PRO_NOMBRE','PRO_DESTINO','PRO_LOGO_PREMIUM','PRO_LOGO_GENERAL','PRO_DESCRIPCION','PRO_URL','CAT_ID','MAR_ID','eventos');
        $crud->columns('PRO_ID','PRO_NOMBRE','PRO_LOGO_PREMIUM','PRO_LOGO_GENERAL','PRO_AUTOR','CAT_ID','SUB_ID','AUTORIZADO','eventos');
      ...
      $crud->field_type('eventos','multiselect', $arrEventos);
		...
		$this->data['titulo']='Promociones Premium Categoría';
		$this->data['encabezado']='Gestión de promociones premium categoría';

		$breadcrums[]='<a class="current" href="'.site_url('main/promociones').'">Promociones</a>';
		$this->salida('promociones_premium/promociones_premium',$this->data, $breadcrums);
	}
...
function delete_motivo_rechazo($id_promo){
	$this->load->model('promociones_premium_model');
	$this->promociones_premium_model->delete_motivo_rechazo($id_promo);
	$this->load->helper('url');

	$result = $this->promociones_premium_model->getPromocionById($id_promo);
	$datos_envio['titulo'] = $result['PRO_NOMBRE'];
	$datos_envio['autor'] = $result['PRO_AUTOR'];
	$datos_envio['aliado'] = $result['PRO_USER_CREADOR'];
  $datos_envio['eventos'] = $this->promociones_premium_model->get_eventos_promocion($id_promo);
	$datos= $this->promociones_premium_model->send_mail_delete_user($datos_envio);
	$this->promociones_premium_model->send_mail_delete_aliado($datos_envio);
}
...
foreach ($post_array['eventos'] as $evento){
  ...
  $datos= $this->promociones_premium_model->decrementar($pUSER_ID,$pTIPO,$evento);
...
function before_update($post_array, $primary_key){
  ...
  $datos_envio['eventos'] = $this->promociones_premium_model->get_eventos_promocion($primary_key);


  //validar limite paquete
  $comprobacion = $this->promociones_premium_model->validar_limite_edit($primary_key,$post_array);
  if($comprobacion!=1){
  ...
  //fin validacion
    ...
    elseif($post_array['VISIBILITY'] == 1 && $visibilidad['VISIBILITY'] == 0){
    	...
    	$this->promociones_premium_model->send_mail_aliado_edit($datos_envio);
    }
...
function fnc_after_update($post_array, $primary_key){ //print_r($post_array);die();
    ...
    $datos_envio['aliado'] = $this->session->userdata('sadmin_user_id');
  $datos_envio['eventos'] = $this->promociones_premium_model->get_eventos_promocion($primary_key);
    $this->promociones_premium_model->send_mail_user_edit($datos_envio);
...
function fnc_after_insert($post_array, $primary_key){
    ...
    $datos_envio['eventos'] = $this->promociones_premium_model->get_eventos_promocion($primary_key);
      $datos= $this->promociones_premium_model->send_mail_user($datos_envio);
      $this->promociones_premium_model->send_mail_aliado($datos_envio);
       // return true;
	return $post_array;
