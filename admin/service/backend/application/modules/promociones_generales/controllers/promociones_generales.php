...
function delete_motivo_rechazo($id_promo){
	$this->load->model('promociones_generales_model');
	$this->promociones_generales_model->delete_motivo_rechazo($id_promo);
	$this->load->helper('url');
...
foreach ($post_array['eventos'] as $evento){
  ...
  $datos= $this->promociones_generales_model->decrementar($pUSER_ID,$pTIPO,$evento);
...
function before_update($post_array, $primary_key){
  ...
  $comprobacion = $this->promociones_generales_model->validar_limite_edit($primary_key,$post_array);
  if($comprobacion!=1){
  ...
  //fin validacion
	...
	$visibilidad = $this->promociones_generales_model->verificar_visibilidad($primary_key);
