...
class image_CRUD {
	...
	protected $default_language_path = 'service/backend/assets/image_crud/languages';

	/* Nuevo */
	protected $width = 1000;
	protected $height = 1000;
	protected $th_width = 125;
	protected $th_height = 125;
	protected $types = array('jpg','jpeg','png','gif');

	protected $image_path_thumbs = '';
	protected $image_path_ampliada = '';
	protected $color_bg = '#FFF';
	/* Nuevo */
	protected $field_extra = null;
	protected $field_extra_value = null;
	/**
	 *
	 * @var Image_moo
	 */
	...
	function set_th_width($width){
		$this->th_width = $width;
	}
	function set_th_height($height){
		$this->th_height = $height;
	}
	/* SETTING COLOR */
		function set_color_bg($color_bg){
...
// NUEVO AMPLIADA Y THUMBS
	function set_image_path_ampliada($image_path)
	{
		$this->image_path_ampliada = $image_path;

		return $this;
	}

	function set_image_path_thumbs($image_path)
	{
		$this->image_path_thumbs = $image_path;
	...
	function set_field_extra($field,$extra = null)
	{
		$this->field_extra = $field;
		$this->field_extra_value = $extra;
	...
	{
		$this->css_files[sha1($css_file)] = base_url().(str_replace('assets/image_crud/','',$css_file));
	}

	public function set_js($js_file)
	{
		$this->js_files[sha1($js_file)] = base_url().(str_replace('assets/image_crud/','',$js_file));
	}
	...
	protected function _library_view($view, $vars = array(), $return = FALSE)
	{
		...
		$view_file = 'service/backend/assets/image_crud/views/';
...
/* NUEVAS APLICACIONES PARA PROYECTO ESPECIFICO . _new_**** */  
	protected function _new_upload_file($path) {
        ...
        fclose($target);
		$this->image_moo->load($path)->save_dynamic($path,true);
        /* ------------------------------------- */
        return true;
    }

	protected function _new_upload_file_ampliada($image_path, $thumbnail_path)
	{
		$this->image_moo->load($image_path)->set_background_colour($this->color_bg)->resize($this->width,$this->height,TRUE)->save($thumbnail_path,true);
	}

	protected function _new_create_thumbnail($image_path, $thumbnail_path)
	{
		$this->image_moo
			->load($image_path)
			->set_background_colour($this->color_bg)
			->resize($this->th_width,$this->th_height,TRUE)
			->save($thumbnail_path,true);
	}
	
/* FIN NUEVAS APLICACIONES PARA PROYECTO ESPECIFICO . _new_**** */ 

	protected function _upload_file($path) {
		$upload_file_ext = end($path);
        ...
        /* Cambiar al tamaño seteado */
			$this->image_moo->load($path)->set_background_colour($this->color_bg)->resize($this->width,$this->height,TRUE)->save_dynamic($path,true);
        /* ------------------------------------- */
        return true;
    }
...
//  	$insert = array($this->url_field => $file_name, $this->field_extra => $this->field_extra_value);
    ...
    protected function _get_photos($relation_value = null)
    {
    	...
    	{
			/*
				ESTA LINEA CREA THUMBS CUANDO ES NECESARIO SOBRE LA MISMA CARPETA - EN ESTE PROYECTO NO HIZO FALTA.
    		...
    		$results[$num]->image_url = base_url().$this->image_path_ampliada.'/'.$row->{$this->url_field};
    		$results[$num]->thumbnail_url = base_url().$this->image_path_thumbs.'/'.$row->{$this->url_field};
	...
	{
		include('service/backend/assets/image_crud/config/translit_chars.php');
...
//				$ext = substr($new_file_name,-3,3);
//				$file_name = uniqid().'-'.strtotime(date("Y-m-d H:i:s")).'-'.date('Ymdhis').'.'.$ext;
			#endregion
		...
		if(!empty($state_info))
		{
			switch ($state_info->name) {
				...
				case 'upload_file':
					...
					$path_info = pathinfo($this->image_path.'/'.$file_name);
					$ext = $path_info['extension'];

					if(in_array($ext,$this->types)){
						$this->_new_upload_file( $this->image_path.'/'.$file_name );
						$this->_new_upload_file_ampliada( $this->image_path.'/'.$file_name , $this->image_path_ampliada.'/'.$file_name );
						$this->_new_create_thumbnail( $this->image_path.'/'.$file_name , $this->image_path_thumbs.'/'.$file_name );
