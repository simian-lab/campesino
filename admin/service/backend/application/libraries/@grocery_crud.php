...
//		echo ">".$callback_return;
//		echo "<textarea>".json_encode(array('error_message' => $callback_return,'success' => false ))."</textarea>";
...
class UploadHandler
{
    ...
    private function handle_file_upload($uploaded_file, $name, $size, $type, $error) {
        ...
        if (!$error && $file->name) {
            ...
            $file->error =    is_file($file_path) && $file->size > filesize($file_path);
            clearstatcache();
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
            ...
            if ($file_size === $file->size) {
                ...
                unlink($file_path);
               // $file->error = 'abort';
//                $file->error = '--abort'.$this->options['discard_aborted_uploads'].'-'.$file_size.' === '.$file->size;
            }
            $file->size = $file_size;
            $file->delete_url = $this->options['script_url']
                .'?file='.rawurlencode($file->name);
