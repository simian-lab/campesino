...
//		$password   = $password;//$this->hash_password($password, $salt);
...
//			$password = $this->hash_password_db($user->id, $password);
			...
			if ($password === TRUE)
			{
				...
				/* Consulta el menu segun el grupo perteneciente */
//				$menu = NULL;
				...
				$session_data = array(
				    ...
				    'email'                => $user->email,
				    'sadmin_user_id'              => $user->id, //everyone likes to overwrite id so we'll use user_id
...
// 'menu'       => ,
	...
	public function user($id = NULL)
	{
		...
		$id || $id = $this->session->userdata('sadmin_user_id');
	...
	public function add_to_group($group_id, $user_id=false)
	{
		...
		//if no id was passed use the current users id
//		$user_id || $user_id = $this->session->userdata('sadmin_user_id');
	...
	**/
	...
	/* NUEVOS - PROCESOS*/
	public function register_process($process, $method, $process_id, $menu, $orden, $style)
	{
		$this->trigger_events('pre_register_process');
		...
		$this->db->insert($this->tables['process'], $data);
	...
	public function edit_process($process, $method, $process_id, $id, $menu, $orden, $style)
	{
		$this->trigger_events('pre_edit_process');
		...
		$this->db->where('id', $id);
		$this->db->update($this->tables['process'], $data);

		$this->trigger_events('post_edit');
	...
	public function get_process()
	{
		...
		return $query = $this->db->get($this->tables['process'])->result_array();		
	}
	
	public function get_process_list()
	{
		$this->db->select(" `t1`.*, IF(t1.menu = '0', 'No','Si') AS menu,`t2`.`process` AS proceso_padre, 
			CONCAT_WS('.', if(t2.orden IS NULL, t1.orden, t2.orden), if(t2.orden IS NULL, 0, t1.orden)) AS ord", false);
		$this->db->from($this->tables['process'].' t1');
		$this->db->join($this->tables['process'].' t2', 't2.id = t1.process_id', 'left');
		$this->db->group_by("t1.id"); 
		$this->db->order_by("ord"); 
		return $query = $this->db->get($this->tables['process'])->result();
	}

	public function get_process_list_groups()
	{
		$this->db->select("t1.process, t1.id,  IF(t1.menu = '0', 'No','Si') AS menu, t1.process_id, 
		CONCAT_WS('.', if(t2.orden IS NULL, t1.orden, t2.orden), if(t2.orden IS NULL, 0, t1.orden)) AS ord", false);
		$this->db->from($this->tables['process'].' t1');
		$this->db->join($this->tables['process'].' t2', 't2.id = t1.process_id', 'left');
		$this->db->group_by("t1.id"); 
	...
	{
		$this->trigger_events('process');

		$this->db->limit(1);
		$this->db->where($this->tables['process'].'.id', $id);
		$this->db->from($this->tables['process']);
	...
	{
		return $query = $this->db->get($this->tables['groups'])->result();
	}

	public function register_groups($name, $description)
	{
		$this->trigger_events('pre_register_groups');
	...
	public function edit_groups($name, $description, $id)
	{
		$this->trigger_events('pre_edit_groups');
	...
	public function add_process_groups_db($values, $groups_id){

		$this->trigger_events('delete_process_to_groups');

		$this->db->where('group_id', $groups_id);
		$this->db->delete($this->tables['process_groups']);

		if($values){
			$this->trigger_events('add_process_groups_db');
	
			$this->db->insert_batch($this->tables['process_groups'], $values); 
	
			$this->trigger_events('added_process_groups_db');
		}
		return true;
		
	}
	public function get_process_groups($id){

		$this->db->select("process_id", false);
		$this->db->where($this->tables['process_groups'].'.group_id', $id);
		return $query = $this->db->get($this->tables['process_groups'])->result();
		
	}
	

	public function consulta_menu($id){
		$this->db->select("t1.process, t1.id,  IF(t1.menu = '0', 'No','Si') AS menu, t1.process_id, t1.method, t1.style,
		CONCAT_WS('.', if(t2.orden IS NULL, t1.orden, t2.orden), if(t2.orden IS NULL, 0, t1.orden)) AS ord", false);
		$this->db->from($this->tables['process'].' t1');
		$this->db->join($this->tables['process'].' t2', 't2.id = t1.process_id', 'left');

		$this->db->join($this->tables['process_groups'].' t3', 't1.id = t3.process_id', 'inner');
		$this->db->join($this->tables['groups'].' t4', 't4.id = t3.group_id', 'inner');
		$this->db->join($this->tables['users_groups'].' t5', 't4.id = t5.group_id', 'inner');
		$this->db->where("t5.user_id", $id); 
		$this->db->group_by("t1.id"); 
		$this->db->order_by("ord"); 
		$query = $this->db->get($this->tables['process'])->result();
//			$str = $this->db->last_query();  
		return $query;
	}
	
	public function delete_process($id){
		$this->db->delete($this->tables['process'], array('id' => $id)); 
	}
	public function verifica_delete_process($id){
	...
	public function checkProcessGroupsUsr($id = null, $process = null){
		$this->db->select("DISTINCT  t4.id", false);
		$this->db->from($this->tables['users'].' t1');
		$this->db->join($this->tables['users_groups'].' t2', 't2.user_id = t1.id', 'inner');

		$this->db->join($this->tables['process_groups'].' t3', 't3.group_id = t2.group_id', 'inner');
		$this->db->join($this->tables['process'].' t4', 't4.id = t3.process_id', 'inner');

		$this->db->where("t1.id", $id); 
		$this->db->where("t4.method", $process); 
		//$query = $this->db->get($this->tables['users'])->result();
	...
	public function update_user($email = null, $id= null){
		$data = array(
               'email' => $email,
               'username' => $email			   
            );
	...
	// Verifico que el acceso que quiere obtener el usuario es permitido.
	public function access($segment, $user){

		$SQL = 'SELECT t1.id
				FROM admin_process t1
					INNER JOIN admin_process_groups t2 ON(t2.process_id = t1.id)
					INNER JOIN admin_groups t3 ON(t3.id = t2.group_id)
					INNER JOIN admin_users_groups t4 ON(t4.group_id = t3.id)
				WHERE t1.method = "'.$segment.'" AND t4.user_id = '.$user.'
				GROUP BY 1';
//				die($SQL);
