				...
				success: function(data){
					...
					if(data.success)
					{
						$('#crudForm').ajaxSubmit({
							...
							cache: 'false',
							beforeSend: function(){
								var str = $('.ftitle-left').text();
								var patt = new RegExp(".*Promociones.*");
								var rr = patt.test(str);
								if(rr){ 
									var r = confirm("\u00bfEst\u00e1 de acuerdo con la vista previa y guardar la promoci\u00f3n?");
									if (r) $("#FormLoading").show();
									return r;
								}
							},
